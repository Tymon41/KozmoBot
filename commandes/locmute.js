exports.run = (client, message, args) => {
  ...
  let mentionned = message.mentions.first;
  ...
  .setAuthor(message.author.username, message.author.avatarURL())
  .setTitle(`Succès`)
  .setDescription(`${mentionned.username} a bien été mute sur ce salon`)
  ...
  message.channel.createOverwrite(mentionned, {
    SEND_MESSAGES: false
  })
  .catch(console.error);
  ...
  console.log(chalk.redBright("MOD: "), `${member.user.tag} locmute par ${message.author.tag} sur ${message.channel.name}`);//Et on log ça comme souvenir
  }
